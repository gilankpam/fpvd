...
- `gs/` — **fpvdgs**, Python ≥3.11, runs on the ground station. Supervises the wfb data plane via native asyncio orchestration (spawning `wfb_rx`/`wfb_tx` directly from `gs/fpvdgs/wfb/`, in-process stats + TX-card selection, supporting remote cards via `link.cards` flat list [SSH-bootstrapped nodes, keys on GS; per-card optional `initScript` runs verbatim on every (re)connect before `iw reg set`, e.g. to create an OpenWrt monitor iface that doesn't survive reboot; `link.wlans` deprecated]), pixelpilot (display), the dynamic-link controller, the beamforming armer, and the IDR relay. The legacy wfb-ng Python runner and the `wfb.engine` selector were removed in Phase 3 — native is the sole runner, no flag. (The GS probe receiver is bypassed — retained in-tree, never constructed; see the probe paragraph below.)
