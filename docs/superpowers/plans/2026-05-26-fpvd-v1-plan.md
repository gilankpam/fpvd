...
TEST_CASE("process: start a child that exits 0, observe clean exit") {
    // /bin/sh -c "exit 0" is portable across systems (NixOS doesn't have /bin/true).
...
void Supervisor::loop() {
    ...
    while (!stopFlag_.load()) {
        ...
        // Increment restart count BEFORE the cap check so restartCount()
        // reflects the failure that tripped the cap (test expects
        // restartCount() >= failureCap when Failed).
...
SubsystemDiff diffSubsystems(const Config& a, const Config& b) {
    ...
    for (auto& [name, sb] : b.services) {
        (void)sb;  // silence -Wunused-variable
...
**Update 2026-05-27:** Added `--log PATH` CLI flag. When set, `main()` opens the file with `O_WRONLY|O_CREAT|O_APPEND` (mode 0644) and `dup2`s it onto fd 1 and fd 2 before bootstrap, so all subsequent `std::cerr`/`std::cout` output (and any child process output that isn't separately redirected) lands in the log file. Used by the init script (Task 21).
...
**Update 2026-05-27:** This script was rewritten post-task to use `start-stop-daemon` (matching OpenIPC convention: S01syslogd, S60crond, S49ntpd) with a PID file at `/var/run/fpvd.pid`, and now passes `--log /tmp/fpvd.log` to the daemon so the daemon's own output lands in the log file. The body below reflects the current version.
