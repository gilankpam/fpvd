...
from fpvdgs.dynlink.drone_config import (
    DroneConfigEvent,
    DroneConfigState,
    State,
)
...
async def test_tunnel_listener_dispatches_hello_to_handler():
    ...
    from fpvdgs.dynlink.tunnel_listener import TunnelListener
    from fpvdgs.dynlink.wire import encode_hello
...
def test_hello_with_vanilla_flag_sets_interleaving_supported_false():
    from fpvdgs.dynlink.wire import Hello, HELLO_FLAG_VANILLA_WFB_NG
    s = DroneConfigState()
    s.on_hello(Hello(generation_id=0xCAFEBABE,
                     mtu_bytes=3994, fps=60,
                     applier_build_sha=0xDEADBEEF,
