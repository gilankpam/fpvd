...
from fpvdgs.dynlink.bitrate import (
    BitrateConfig, compute_bitrate_kbps, compute_wire_target_kbps, effective_phy_Mbps,
)
from fpvdgs.dynlink.profile import load_profile_file
...
def _profile():
    return load_profile_file(PACKAGED_DIR / "m8812eu2.json")
...
def test_compute_wire_target_warns_once_when_preamble_missing(caplog, monkeypatch):
    ...
    import logging
    monkeypatch.setattr("fpvdgs.dynlink.bitrate._warned_missing_preamble", set())
    p = load_profile_file(PACKAGED_DIR / "m8812eu2.json")
    p = replace(p, preamble_us_per_frame=None, name="m8812eu2-noprmbl-1")
    caplog.set_level(logging.WARNING, logger="fpvdgs.dynlink.bitrate")
