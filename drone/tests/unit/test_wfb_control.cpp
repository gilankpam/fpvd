...
TEST_CASE("wfb_control: setRadio sends correct wire bytes, parses ok reply") {
    ...
    ssize_t n = ::recvfrom(srv, &req, sizeof(req), 0, reinterpret_cast<sockaddr*>(&from), &flen);
    REQUIRE(n == static_cast<ssize_t>(offsetof(fpvd::WfbCmdReq, u) + sizeof(req.u.set_radio)));
    ...
    resp.rc = htonl(0);
    ::sendto(srv, &resp, offsetof(fpvd::WfbCmdResp, u), 0, reinterpret_cast<sockaddr*>(&from),
             flen);
