...
LinkChange classifyLinkChange(const Config& a, const Config& b) {
    ...
    c.videoRadiotap = (la.mcs != lb.mcs) || (la.stbc != lb.stbc) || (la.ldpc != lb.ldpc) || width;
    const bool fecMode = la.fec.mode != lb.fec.mode;
    // Per-mode param diff: only the active mode's knobs are live.
    c.videoFec =
        !fecMode && (lb.fec.mode == "swfec" ? (la.fec.overheadPct != lb.fec.overheadPct) ||
