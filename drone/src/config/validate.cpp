...
std::vector<ValidationError> validate(const Config& c) {
    ...
    if (c.link.beamforming.enabled) {
        ...
        if (c.link.stbc)
            errs.push_back({"link.beamforming", "requires link.stbc=false"});
        if (!isValidMac(bf.remoteMac))
            errs.push_back(
                {"link.beamforming.remoteMac", "must be a valid MAC (aa:bb:cc:dd:ee:ff)"});
    ...
    static const std::set<std::string> resiliencePresets{"off",    "rescue",    "quality", "sprint",
                                                         "racing", "endurance", "patrol",  "rally",
                                                         "range",  "fpv"};
    if (!resiliencePresets.count(c.video.resilience))
        errs.push_back({"video.resilience", "must be one of off|rescue|quality|sprint|racing|"
    ...
    // telemetry
    if (c.telemetry.router != "msposd" && c.telemetry.router != "mavfwd" &&
    ...
    for (auto& [name, s] : c.services) {
        if (!restartModes.count(s.restart))
            errs.push_back({"services." + name + ".restart", "must be always|on-failure|never"});
        if (s.exec.empty())
    ...
    // dynamicLink
    {
        ...
        if (dl.roiQp.thresholdKbps <= 0 || dl.roiQp.lowAnchorKbps <= 0 ||
            dl.roiQp.thresholdKbps <= dl.roiQp.lowAnchorKbps)
            errs.push_back({"dynamicLink.roiQp", "require thresholdKbps > lowAnchorKbps > 0"});
        if (dl.roiQp.floor > 0)
        ...
        if (dl.compute.minBitrateKbps <= 0 ||
            ...
            errs.push_back({"dynamicLink.compute", "require 0 < minBitrateKbps < maxBitrateKbps"});
