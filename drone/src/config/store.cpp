...
static void collectUnknown(const nlohmann::json& cfg, const nlohmann::json& ref,
    ...
    if (!cfg.is_object() || !ref.is_object())
        return;
    for (auto it = cfg.begin(); it != cfg.end(); ++it) {
        std::string path = prefix.empty() ? it.key() : prefix + "." + it.key();
        if (prefix.empty() && it.key() == "services")
            continue; // free-form map
        if (!ref.contains(it.key())) {
            out.push_back(path);
            continue;
        }
...
Config loadEffective(const std::string& configPath) {
    ...
    nlohmann::json fileJ;
    try {
        fileJ = nlohmann::json::parse(buf.str());
...
void atomicWriteJson(const std::string& path, const nlohmann::json& j) {
    ...
    if (fd < 0)
        throw StoreError("open .tmp: " + std::string(strerror(errno)));
