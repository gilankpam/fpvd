...
std::vector<SupervisedSpec> buildProbeSpecs(const Config& c, const std::string& iface,
    ...
    tx.argv = {
        ...
        "1",
        "-C",
        std::to_string(kProbeControlPort),
        "-i",
        std::to_string(c.link.linkId),
        "-p",
        std::to_string(kProbeRadioPort),
        "-u",
        std::to_string(kProbeFeedPort),
        iface,
    };
    ...
    SupervisedSpec fd{};
    fd.name = "probe-feed";
    fd.argv = {feederPath, std::to_string(kProbeFeedPort), std::to_string(kProbePps),
               std::to_string(kProbePacketBytes)};
