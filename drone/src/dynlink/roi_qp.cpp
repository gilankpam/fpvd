...
int computeRoiQp(uint16_t bitrateKbps, uint16_t thresholdKbps, uint16_t lowAnchorKbps, int8_t floor,
                 uint8_t step) {
    if (bitrateKbps >= thresholdKbps)
        return 0;
    ...
    if (q < static_cast<int>(floor))
        q = static_cast<int>(floor);
    if (q > 0)
        q = 0;
    return q;
}
