...
EncoderClient::EncoderClient(WaybeamClient& client, RoiCurve roi) : client_(&client), roi_(roi) {}

int EncoderClient::apply(uint16_t bitrateKbps, uint8_t fps) {
    if (bitrateKbps == 0)
        return 0; // sentinel: don't push

    auto roiQp = static_cast<int8_t>(
        computeRoiQp(bitrateKbps, roi_.thresholdKbps, roi_.lowAnchorKbps, roi_.floor, roi_.step));

    if (lastValid_ && lastBitrate_ == bitrateKbps && lastRoiQp_ == roiQp && lastFps_ == fps) {
...
int EncoderClient::applySafe(uint16_t safeBitrateKbps) {
    auto roiQp = static_cast<int8_t>(computeRoiQp(safeBitrateKbps, roi_.thresholdKbps,
                                                  roi_.lowAnchorKbps, roi_.floor, roi_.step));
