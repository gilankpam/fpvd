...
int parseCbrRssi(const std::string& rfinfo) {
    ...
    std::string tok =
        rfinfo.substr(start, end == std::string::npos ? std::string::npos : end - start);
    try {
        return std::stoi(tok);
    } catch (...) {
        return 0;
...
BeamformingController::BeamformingController(std::string procBase, std::string sysBase)
...
bool BeamformingController::writeNode(const std::string& iface, const std::string& node,
...
void BeamformingController::reconcile(bool enabled, const BfParams& p, bool force) {
    ...
    if (!supported(p.iface)) {
        ...
        s.reason = "no bf_monitor proc node on " + p.iface + " (driver " + p.driver + ")";
        std::lock_guard<std::mutex> g(mu_);
        status_ = s;
        params_ = p;
        running_ = false;
    ...
    bool ok = writeNode(p.iface, "bf_monitor_conf", "1 " + p.remoteMac + " 0 0");
...
void BeamformingController::loop() {
    ...
    while (!stopFlag_.load()) {
        ...
        trig += " " + p.remoteMac + " 0 0 " + std::to_string(token) + " " + std::to_string(bw);
        ...
        if (!ok) {
            ...
            if (cbrTok != lastCbrToken) {
                lastCbrToken = cbrTok;
                cbrStaleTicks = 0;
            } else if (cbrStaleTicks < kCbrStaleTicks) {
                ++cbrStaleTicks;
            }
            ...
            status_.lastCbr = cbr.empty() ? std::nullopt : std::optional<std::string>(cbr);
