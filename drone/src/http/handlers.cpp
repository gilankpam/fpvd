...
void registerHandlers(HttpServer& srv, Daemon& d, bool reallyRestart) {
    ...
    srv.get("/config", [&](const httplib::Request& req, httplib::Response& res) {
        ...
        nlohmann::json j = pending ? nlohmann::json(d.pending()) : nlohmann::json(d.effective());
    ...
    srv.patch("/config", [&](const httplib::Request& req, httplib::Response& res) {
        ...
        if (!pr.ok) {
            if (!pr.lockedPaths.empty()) {
                ...
                res.status = 400;
                res.set_content(
                    errBody("dynamic_link_locked",
                            "fields owned by the dynamic-link controller while dynamicLink.enabled",
                            details)
                        .dump(),
                    "application/json");
                return;
    ...
    srv.post("/apply", [&, reallyRestart](const httplib::Request&, httplib::Response& res) {
        ...
        if (!ar.ok) {
            ...
            for (auto& e : ar.errors)
                details.push_back({{"path", e.path}, {"message", e.message}});
            res.status = 400;
            res.set_content(errBody("validation", "cannot apply invalid config", details).dump(),
                            "application/json");
            return;
        }
        nlohmann::json out = {
            {"applied", true}, {"version", ar.version}, {"restarted", ar.restarted}};
