...
static std::vector<std::string> commonTx(const Config& c, int mcs, const std::string& /*iface*/,
    ...
    if (c.link.fec.mode == "swfec") {
        ...
        a.push_back("-z");
        a.push_back("-k");
        a.push_back(std::to_string(c.link.fec.overheadPct));
        a.push_back("-n");
        a.push_back(std::to_string(c.link.fec.deadlineMs));
    } else {
        a.push_back("-k");
        a.push_back(std::to_string(c.link.fec.k));
        a.push_back("-n");
        a.push_back(std::to_string(c.link.fec.n));
    }
    a.insert(a.end(), {"-S", c.link.stbc ? "1" : "0", "-L", c.link.ldpc ? "1" : "0", "-i",
                       std::to_string(c.link.linkId)});
    return a;
}
...
std::vector<std::string> wfbArgs(const Config& c, WfbRole role, const std::string& iface,
    ...
    case WfbRole::VideoTx: {
        ...
        a.push_back("-U");
        a.push_back("venc_wfb");
        a.push_back("-C");
        a.push_back(std::to_string(kVideoControlPort));
        a.push_back("-J");
        a.push_back("10");
        a.push_back("-E");
        a.push_back("5000");
    ...
    case WfbRole::TlmRx:
        return {
            "/usr/bin/wfb_rx", "-K", key, "-i", std::to_string(c.link.linkId), "-p", "144", "-u",
            "14550",           iface};
    case WfbRole::TlmTx: {
        auto a = tunTlmTx(c, key);
        a.push_back("-p");
...
std::vector<std::string> wfbTunArgs() { return {"/usr/bin/wfb_tun", "-a", "10.5.0.10/24"}; }
