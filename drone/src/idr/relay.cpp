...
IdrRelay::IdrRelay(WaybeamClient& enc, std::string bindAddr, uint16_t port,
...
int IdrRelay::requestIdr(uint64_t nowMs) {
    if (everSent_ &&
        (nowMs - lastIdrMs_) < static_cast<uint64_t>(minIntervalMs_)) {
        return 1;  // throttled
    }
...
void IdrRelay::start() {
    if (listener_.fd() < 0) return;     // disabled — nothing to listen to
...
void IdrRelay::run() {
    ...
    pfds[0].fd = listener_.fd(); pfds[0].events = POLLIN;
    pfds[1].fd = wakeFd_;        pfds[1].events = POLLIN;
