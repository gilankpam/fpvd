...
TEST_CASE("wfb_control: setInterleaveDepth sends correct wire bytes, parses ok reply") {
    uint16_t port = 0;
    int srv = bindServer(port);
    ...
    ssize_t n = ::recvfrom(srv, &req, sizeof(req), 0,
                           reinterpret_cast<sockaddr*>(&from), &flen);
    REQUIRE(n == static_cast<ssize_t>(offsetof(fpvd::WfbCmdReq, u) +
                                      sizeof(req.u.set_interleave_depth)));
    CHECK(req.cmd_id == fpvd::kWfbCmdSetInterleaveDepth);
    CHECK(req.u.set_interleave_depth.depth == 7);
...
TEST_CASE("wfb_control: setInterleaveDepth non-zero rc surfaces as error") {
    uint16_t port = 0;
    int srv = bindServer(port);
    ...
    ::recvfrom(srv, &req, sizeof(req), 0,
               reinterpret_cast<sockaddr*>(&from), &flen);
    CHECK(req.cmd_id == fpvd::kWfbCmdSetInterleaveDepth);
    CHECK(req.u.set_interleave_depth.depth == 4);
...
TEST_CASE("wfb_control: setInterleaveDepth silence yields timeout error") {
    uint16_t port = 0;
    int srv = bindServer(port);
    fpvd::WfbControlClient c("127.0.0.1", port);
    auto res = c.setInterleaveDepth(3);   // server never replies (~500 ms)
    CHECK_FALSE(res.ok);
    CHECK(res.error.find("timeout") != std::string::npos);
    ::close(srv);
}
